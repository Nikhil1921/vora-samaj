...
class Video_gallery extends Admin_controller  {
	...
	private $table = 'video_gallery';
	protected $redirect = 'video_gallery';
	protected $title = 'Video gallery';
	protected $name = 'video_gallery';
    ...
    {
        check_ajax();
        $this->load->model('video_gallery_model', 'data');
	...
	public function update($id)
	{
        ...
        if ($this->form_validation->run() == FALSE)
        {
            ...
            $data['url'] = $this->redirect;
            $data['kacheries'] = $this->main->getAll('kacheries', 'id, CONCAT(name, " કચેરી") name', ['is_deleted' => 0]);
            $data['data'] = $this->main->get($this->table, 'k_id, name, v_url', ['id' => d_id($id)]);
